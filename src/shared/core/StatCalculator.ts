...
export class StatCalculator {
  public static calculate(player: SharedPlayer, stat: Stat): number {
    ...
    for (const item of player.equipment.values()) {
      ...
      if (modifier) {
        if (modifier.type === "FLAT") flatAdd += modifier.value;
        if (modifier.type === "PERCENT") percentAdd += modifier.value;
      }
    }
