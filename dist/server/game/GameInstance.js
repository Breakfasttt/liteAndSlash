...
import { Ennemi, WeaponType } from "../../shared/core/BaseClasses";
...
import { checkCircleCollision, checkCircleAABBCollision } from "../../shared/physics/types";
...
export class GameInstance {
    ...
    aoeZones = [];
    rooms = new Map();
    activeRoomId = "room_start";
    keysCollected = 0; // Clé 1 (demi-boss rouge) et Clé 2 (demi-boss bleu)
    // Couloirs et Portes (Map continue)
    corridors = [];
    doors = [];
    // Grille physique
    grid = new SpatialHashGrid(128);
    // Tickrate
    intervalId = null;
    isEnded = false;
    startTime = 0;
    constructor(id, seed, bossId, difficulty, clients) {
        this.id = id;
        this.seed = seed;
        this.bossId = bossId;
    ...
    connectRooms(r1Id, r2Id, direction, locked = false) {
        ...
        if (locked) {
            ...
            let color = "boss";
            if (r1Id === "room_demiboss_red" || r2Id === "room_demiboss_red")
                color = "red";
            else if (r1Id === "room_demiboss_blue" || r2Id === "room_demiboss_blue")
                color = "blue";
            this.doors.push({
                id: `door_${corridorId}`,
                x: doorX,
    ...
    async start() {
        ...
        for (const p of this.players.values()) {
            ...
            if (char) {
                ...
                p.client.initClass(char.classShape);
    ...
    tick() {
        ...
        this.grid.clear();
        for (const p of this.players.values()) {
            if (!p.isKO && !p.client.isDead)
                this.grid.insert(p.client);
        }
    ...
    resolveCollisionsForEntity(entity, isDashing) {
    ...
    resolveCircleAABBCollision(circle, rect) {
        ...
        for (const p of this.players.values()) {
            ...
            for (const slot of ["ARME1", "ARME2"]) {
                ...
                if (p[timerKey] <= 0) {
                    ...
                    if (closestEnemy) {
                        p[timerKey] = wInstance.autoAttackNode.cooldown;
    ...
    castUltimate(p, slot) {
        if (slot !== "ARME1")
            return; // L'ultime n'est lançable que par l'Arme 1
        const item = p.client.equipment.get(slot);
        if (!item)
            return;
    ...
    // ==========================================
    calculateWeaponDamage(player, weapon) {
    ...
    broadcastBulletPattern(pattern) {
    ...
    applyDamageToPlayer(p, damage) {
        if (p.isKO || p.client.isDead || p.dashDuration > 0)
            return; // Invulnérable pendant le dash
    ...
    applyMeleeDamage(enemy, target, damage) {
    ...
    // ==========================================
    spawnEnemyInRoom(roomId, enemyType) {
    ...
    getEnemyCountInRoom(roomId) {
        return this.enemies.size;
    }
    unlockDoorsOfRoom(roomId) {
    ...
    async handleCraftAction(userId, payload, ws) {
        ...
        if (action === "SALVAGE") {
            ...
            if (item) {
                ...
                if (linesCount + 1 === 2)
                    item.rarity = "GREEN";
                else if (linesCount + 1 === 3)
                    item.rarity = "BLUE";
                else if (linesCount + 1 === 4)
                    item.rarity = "PURPLE";
    ...
    async handleTalentAction(userId, payload, ws) {
        ...
        dummyPlayer.initClass(character.classShape);
        // Charger talents actuels
        const currentTalents = new Set();
        // Simuler le stockage
    ...
    async sendUpdatedCharacter(userId, ws) {
        const character = await dbService.getCharacters(userId).then(list => list[0]);
        ws.send(JSON.stringify({
