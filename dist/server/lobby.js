...
export class LobbyServer {
    ...
    static matchmakingQueues = new Map(); // key: "bossId_difficulty" -> Array of client userId
    static parties = new Map(); // partyId -> Party details
    ...
    static triggerQueueStart(queueKey) {
        const queue = this.matchmakingQueues.get(queueKey);
    ...
    // DIFFUSION COMMUNE
    static broadcastMessage(sender, text, channel) {
    ...
    static broadcastSystemMessage(text) {
        this.broadcastMessage("Système", text, "Général");
    }
    static broadcastHubState() {
    ...
    static broadcastMatchmakingStatus(queueKey) {
        const queue = this.matchmakingQueues.get(queueKey);
    ...
    static broadcastPartyUpdate(partyId) {
        const party = this.parties.get(partyId);
        if (!party)
            return;
    ...
    static broadcastToParty(partyId, sender, text) {
        const party = this.parties.get(partyId);
        if (!party)
            return;
        const message = JSON.stringify({
    ...
    static sendWhisper(client, targetPseudo, text) {
